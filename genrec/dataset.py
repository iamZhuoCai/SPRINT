...
class AbstractDataset:
    ...
    def _leave_one_out(self):
        ...
        max_users = min(self.max_users, len(self.all_item_seqs))
        self.log(f"Using {max_users} users for training")
        ...
        for user in self.all_item_seqs:
            ...
            if len(self.all_item_seqs[user]) > 1:
                ...
                datasets['val']['item_seq'].append(
                    self.all_item_seqs[user][:-1])
            if len(self.all_item_seqs[user]) > 2:
                datasets['train']['user'].append(user)
                datasets['train']['item_seq'].append(
                    self.all_item_seqs[user][:-2])
    ...
    def split(self):
        ...
        # last-out: last item is test, second-to-last is val.
