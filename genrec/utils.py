...
from genrec.constants import MAX_ITEM_SEQ_LEN
...
def init_logger(config: dict):
    ...
    result_dir = os.path.join(
        result_dir, f"{config['model']}_Seq-{MAX_ITEM_SEQ_LEN}_OPQ")
...
def _parse_cli_value(v: str):
    """Parse a CLI string value; tolerate shell-stripped list quotes like [ndcg,recall]."""
    try:
        new_v = eval(v)
        ...
        if v.lower() in ['true', 'false']:
            return v.lower() == 'true'
        # Bash often strips quotes from --metrics=['ndcg','recall'] → [ndcg,recall]
